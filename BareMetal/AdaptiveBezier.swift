...
class QuadraticBezierTesselator {
    var points: [[Float]] = []

    var curve_distance_epsilon: Float = 1e-30
    var curve_collinearity_epsilon: Float = 1e-30
    var curve_angle_tolerance_epsilon: Float = 0.01
    var curve_recursion_limit = 32
    var m_approximation_scale: Float = 100.0
    var m_distance_tolerance_square: Float = 0.5
    var m_angle_tolerance: Float = 0.0

    let thickness: Float
    let miterLimit: Float

    init(start: [Float], c: [Float], end: [Float], options: BezierTesselationOptions) {
        ...
        bezier(x1: x1, y1: y1, x2: x2, y2: y2, x3: x3, y3: y3)
    }

    func calc_sq_distance(x1: Float, y1: Float, x2: Float, y2: Float) -> Float {
        let dx = x2 - x1
        let dy = y2 - y1
        return dx * dx + dy * dy
    }

    func recursive_bezier(x1: Float, y1: Float, x2: Float, y2: Float, x3: Float, y3: Float, level: Int) {
        ...
        if d > curve_collinearity_epsilon {
            ...
            da = dx * dx + dy * dy
            if da == 0 {
                d = calc_sq_distance(x1: x1, y1: y1, x2: x2, y2: y2)
            } else {
                ...
                if d <= 0 { d = calc_sq_distance(x1: x2, y1: y2, x2: x1, y2: y1) }
                else if d >= 1 { d = calc_sq_distance(x1: x2, y1: y2, x2: x3, y2: y3) }
                else { d = calc_sq_distance(x1: x2, y1: y2, x2: x1 + d * dx, y2: y1 + d * dy) }
            }
        ...
        // ----------------------
        recursive_bezier(x1: x1, y1: y1, x2: x12, y2: y12, x3: x123, y3: y123, level: level + 1)
        recursive_bezier(x1: x123, y1: y123, x2: x23, y2: y23, x3: x3, y3: y3, level: level + 1)
    }

    // ------------------------------------------------------------------------
    func bezier(x1: Float, y1: Float, x2: Float, y2: Float, x3: Float, y3: Float) {
        points.append([x1, y1])
        recursive_bezier(x1: x1, y1: y1, x2: x2, y2: y2, x3: x3, y3: y3, level: 0)
        points.append([x3, y3])
    }
...
class CubicBezierTesselator {
    let RECURSION_LIMIT: Int = 8
    let PATH_DISTANCE_EPSILON: Float = 1.0
    let curveAngleToleranceEpsilon: Float
    let mAngleTolerance: Float
    let mCuspLimit: Float
    let thickness: Float
    let miterLimit: Float
    var points: [[Float]]

    init(start: [Float], c1: [Float], c2: [Float], end: [Float], existingPoints: [[Float]], options: BezierTesselationOptions) {
        ...
        curveAngleToleranceEpsilon = options.curveAngleToleranceEpsilon
        mAngleTolerance = options.mAngleTolerance
        mCuspLimit = options.mCuspLimit
        thickness = options.thickness
        miterLimit = options.miterLimit

        var distanceTolerance = PATH_DISTANCE_EPSILON / options.scale
        distanceTolerance *= distanceTolerance
        begin(start: start, c1: c1, c2: c2, end: end, distanceTolerance: distanceTolerance)
    }

    func begin(start: [Float], c1: [Float], c2: [Float], end: [Float], distanceTolerance: Float) {
        points.append(start)
        recursive(x1: start[0], y1: start[1], x2: c1[0], y2: c1[1], x3: c2[0], y3: c2[1], x4: end[0], y4: end[1], distanceTolerance: distanceTolerance, level: 0)
        points.append(end)
    }

    func recursive(x1: Float, y1: Float, x2: Float, y2: Float, x3: Float, y3: Float, x4: Float, y4: Float, distanceTolerance: Float, level: Int) {
        ...
        // Continue subdivision
        recursive(x1: x1, y1: y1, x2: x12, y2: y12, x3: x123, y3: y123, x4: x1234, y4: y1234, distanceTolerance: distanceTolerance, level: level + 1)
        recursive(x1: x1234, y1: y1234, x2: x234, y2: y234, x3: x34, y3: y34, x4: x4, y4: y4, distanceTolerance: distanceTolerance, level: level + 1)
    }
