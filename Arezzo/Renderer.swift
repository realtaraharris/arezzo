...
//  Arezzo
//
//  Created by Max Harris on 4/18/21.
...
//    let vertexDescriptor: MDLVertexDescriptor
    ...
    init(view v: MTKView, device: MTLDevice, drawOperationCollector: DrawOperationCollector) {
        self.device = device
        super.init()
        ...
        self.commandQueue = device.makeCommandQueue() // this is expensive to create, so we save a reference to it
        print("v.frame.width, v.frame.height:", v.frame.width, v.frame.height)
        self.width = v.frame.width
        self.height = v.frame.height
    }
    ...
    func draw(in view: MTKView) {
        ...
        self.generateVerts(endTimestamp: self.endTimestamp)
...
//        #else
        commandBuffer.present((view.currentDrawable?.layer.nextDrawable())!)
//        #endif
...
//         captureManager.stopCapture()
    ...
    final func generateVerts(endTimestamp: Double) {
        ...
        let uniform = Uniforms(width: Float(self.width), height: Float(self.width), modelViewMatrix: modelViewMatrix)
